...
- 인터넷이 되지 않는 환경에서는 다음 이미지가 필요하며, 해당 이미지를 OpenShift 환경에 ImageStream으로 Import 합니다.
  ...
  - 이미지 리스트
    ...
    ```bash
    skopeo copy --dest-creds ${USER}:${PASSWORD} --dest-tls-verify=false docker://docker.io/openshiftdemos/gogs:latest docker://${REGISTRY_HOST}:${PORT}/gogs:stable
  ...
  - 이미지 임포트
    ...
    ```bash
    oc import-image rhel8/postgresql-12:1-170 --from=registry.redhat.io/rhel8/postgresql-12:1-170 --confirm --insecure -n openshift
    oc import-image gogs/gogs:stable --from=docker.io/openshiftdemos/gogs:latest  --confirm --insecure -n openshift
    oc import-image gogs/gogs:stable --from=docker.io/openshiftdemos/gogs:latest  --confirm --insecure -n gogs
    ```
  ...
  - kind: ImageStream
    ...
    spec:
      ...
      - name: "${GOGS_VERSION}"
        from:
          kind: DockerImage
          name: ocp-registry-quay-quay.apps.openshift01.sandbox322.opentlc.com/gogs/gogs:${GOGS_VERSION}
        importPolicy: {}
        annotations:
          tags: gogs,go,golang
